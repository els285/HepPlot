...
class HEP_Plot:
    ...
    def Choose_Design(self,design,**kwargs):

        allowed_experiment_styles = ["ATLAS","ALICE","LHCb2","CMS","ATLASTex"]

        if design=="ATLAS":
            self.Initialise_ATLAS_Plot()
        elif any([design == x for x in allowed_experiment_styles]):
            self.Initialise_LHC_Plot(design)
        elif design=="Seaborn":
            self.Initialise_Seaborn_Plot(**kwargs)

        # switcher = {
        #     "ATLAS"  : self.Initialise_ATLAS_Plot()        ,   
        #     "CMS"    : self.Initialise_LHC_Plot("CMS")     ,
        #     "LHCb2"  : self.Initialise_LHC_Plot("LHCb2")   ,
        #     "ALICE"  : self.Initialise_LHC_Plot("ALICE")   ,
        #     "Seaborn": self.Initialise_Seaborn_Plot()
        # }
    ...
    def __init__(self,plot_title,list_of_histograms,**kwargs):
        self.plot_title = plot_title
        self.list_of_histograms = list_of_histograms
...
class Ratio_Plot(HEP_Plot):
    ...
    def __init__(self,plot_title,list_of_histograms,divisor):
        super().__init__(plot_title,list_of_histograms) 
    ...
    def Compute_Ratio(hist1,hist2):
        ...
        return Ratio_Histogram(ratio_hist,ratio_uncertainties[0],ratio_uncertainties[1])
...
def main():
    ...
    # fig, (ax, rax) = plt.subplots(2, 1, figsize=(6,6), gridspec_kw=dict(height_ratios=[3, 1], hspace=0.1), sharex=True)
    x = Ratio_Plot("plot1",[H_mu_pt,H_mu_E],divisor=H_mu_E)
